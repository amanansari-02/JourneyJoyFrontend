...
function PropertyEditForm() {
    ...
    const getPropertyById = async () => {
        try {
            ...
            if (res?.data?.status === HttpStatusCode.Ok) {
                ...
                setValue('rooms', propertyData.rooms);
                setValue('city', propertyData.city);
                setValue('noOfGuests', propertyData.noOfGuests);

                // Set existing images if exists
                if (propertyData.propertyImages) {
                    const existingImages = propertyData.propertyImages.map((imageUrl) => ({
                        data_url: apiUrl + '/' + imageUrl
                    }));
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const fetchExistingImagesAsFiles = async () => {
                return await Promise.all(
                    images
                        .filter((img) => !img.file)
                        .map(async (img) => {
                            const response = await fetch(img.data_url);
                            const blob = await response.blob();
                            const fileName = img.data_url.split('/').pop();
                            return new File([blob], fileName, { type: blob.type });
                        })
                );
            };

            const existingImagesFiles = await fetchExistingImagesAsFiles();

            for (const key in data) {
                formData.append(key, data[key]);
            }

            existingImagesFiles.forEach((file, index) => {
            ...
            const token = localStorage.getItem('token')
            const res = await PropertyServices.updateProperty(id, formData, token)
            if (res?.data?.status == HttpStatusCode.Ok) {
